    ...
    def corridor(self) -> str:
        ...
        for stop in self.stops:
            ...
            if city and city not in seen:
                ...
                cities.append(city)
        start_city = (
            self.start_address.split(",")[1].strip()
            if "," in self.start_address
            else ""
        )
...
def geocode_all_addresses(addresses: list, progress_cb=None) -> dict:
    ...
    bad_addresses = [
        a for a in addresses if needs_geocode(a) and a.strip().lower() in cache
    ]
    if bad_addresses:
        rcache = _load_json(ROUTECACHE_FILE)
        removed_routes = 0
...
def parse_students_csv(csv_text: str) -> list:
    ...
    for row in reader:
        ...
        try:
            ...
            idx = 0
        last = (
            row.get("Last name") or row.get("last_name") or row.get("Last Name") or ""
        )
        first = (
            row.get("First name")
        ...
        addr = (
            row.get("Primary family address 1")
            or row.get("Address")
            or row.get("address")
            or row.get("Street")
            or ""
        ...
        city = (
            row.get("Primary family city") or row.get("City") or row.get("city") or ""
        )
        zip_ = (
            row.get("Primary family zip")
            or row.get("Zip")
            or row.get("zip")
            or row.get("ZIP")
...
def parse_vehicles_text(text: str) -> list:
    ...
    RNG = re.compile(
        r"^(?:vehicles?\s*)?([A-Z])[-]([A-Z])(?:\s*\(\d+\s*vehicles?\))?", re.IGNORECASE
    )
    expanded = []
    for line in merged:
        m = RNG.match(line)
...
def _sequence_stops_camp_directional(
    stops: list, camp_lat: float, camp_lon: float, trip_direction: str = "morning"
) -> list:
    """
    ...
    while unvisited_list:
        ...
        if eligible:
            ...
            next_stop = min(
                eligible, key=lambda s: haversine_mi(last.lat, last.lon, s.lat, s.lon)
            )
        else:
            # All remaining stops would be a backtrack.
            # Pick the one that backtracks least (closest to current d2c).
            next_stop = min(unvisited_list, key=lambda s: abs(d2c(s) - last_d2c))
...
def cluster_and_route(
    ...
    # -- 1. Geocode -----------------------------------------------------------
    all_addrs = list(
        {s.full_address for s in students}
    ...
    def uc(u):
        return u[0].lat, u[0].lon

    def d2c(u):
        return haversine_mi(*uc(u), camp_lat, camp_lon)
    ...
    if progress_cb:
        progress_cb(
            f"Formed {len(clusters)} geographic clusters "
    ...
    assignable.sort(
        key=lambda cl: haversine_mi(*centroid(cl), camp_lat, camp_lon), reverse=True
    )
    ...
    def nearest_vehicles(clat, clon):
        return sorted(
            ...
            key=lambda vi: haversine_mi(
                clat, clon, veh_objects[vi].start_lat, veh_objects[vi].start_lon
            ),
        )
    ...
    while changed and passes < 30:
        ...
        for vi_dst in range(len(veh_objects)):
            if vi_dst == vi_src:
                continue
            if veh_objects[vi_dst].capacity - counts[vi_dst] < total_src:
                continue
            ...
            if d < full_dist:
                full_dist, full_dest = d, vi_dst

        MAX_MERGE_MI = 5.0
        if full_dest is not None and full_dist <= MAX_MERGE_MI:
            for u in units_src:
                assignments[full_dest].append(u)
            counts[full_dest] += total_src
            assignments[vi_src] = []
            counts[vi_src] = 0
            if progress_cb:
                progress_cb(
                    f"  Merged {veh_objects[vi_src].name} ({total_src}) -> "
        ...
        while remaining:
            ...
            while changed_inner:
                ...
                for unit in list(remaining):
                    ulat, ulon = uc(unit)
                    if any(
                        haversine_mi(ulat, ulon, *uc(g)) <= NEIGHBOR_MI for g in group
                    ):
                        group.append(unit)
        ...
        for group in sub_groups:
            ...
            for vi_dst in range(len(veh_objects)):
                if vi_dst == vi_src:
                    continue
                if veh_objects[vi_dst].capacity - counts[vi_dst] < group_size:
                    continue
                if assignments[vi_dst]:
                    d = min(
                        haversine_mi(glat, glon, *uc(eu)) for eu in assignments[vi_dst]
                    )
                    if d > MAX_SCATTER_MI:
                        continue
                else:
                    ...
                    if d > MAX_SCATTER_MI * 2:
                        continue
                if d < best_d:
                    best_d, best_vi = d, vi_dst
    ...
    for vi, veh in enumerate(veh_objects):
        ...
        for unit in assignments[vi]:
            rep = unit[0]
            key = rep.full_address.lower().strip()
            if key not in addr_stop:
                addr_stop[key] = Stop(
                    address=rep.full_address, lat=rep.lat, lon=rep.lon
                )
...
def build_dashboard(
    ...
    direction_label = (
        "All vehicles depart from"
        if trip_direction == "afternoon"
        else "All vehicles finish at"
    )
    c.value = (
        f"{direction_label}: {camp_display} | "
    ...
    for ci, h in enumerate(hdrs, 1):
        ...
        cell.border = _bdr(
            bottom=MED_SIDE, top=MED_SIDE, left=THIN_SIDE, right=THIN_SIDE
        )
    ...
    for ri, veh in enumerate(vehicles):
        ...
        bg = (
            _fill(ORANGE_FILL)
            if warn
            else (_fill(WHITE) if ri % 2 == 0 else _fill(LIGHT_GRAY))
        )
    ...
    c = ws[f"A{tr}"]
    c.value = (
        f"TOTAL ({total_riders} riders / {total_cap} capacity "
    ...
    if has_warnings:
        c.value = (
            "! Orange rows are below utilization threshold. "
        ...
        c.value = (
            "All vehicles at target capacity | "
            "Drive times via Google Maps | "
...
def build_vehicle_sheet(
    ...
    c.value = (
        f"Start: {veh.start_address} | Cap: {veh.capacity} | "
        f"Riders: {veh.rider_count} ({veh.utilization_pct}%) | "
        f"Kids Ride Time: {veh.total_time}, {veh.total_distance}{warn_tag}"
    )
    c.font = _font(
        size=9, italic=True, color="7B3F00" if veh.under_threshold else DARK_TEXT
    )
    ...
    for ci, h in enumerate(
        ["Stop #", "Address", "Riders", "# Riders", "Drive Time"], 1
    ):
        cell = ws.cell(row=3, column=ci, value=h)
        cell.font = _font(bold=True, size=10, color=WHITE)
        cell.fill = _fill(BRAND_COLOR)
    ...
    for ci, val in enumerate(
        [" START", veh.start_address, "Departure point", None, None], 1
    ):
    ...
    for si, stop in enumerate(veh.stops):
        ...
        for ci, val in enumerate(
            [si + 1, stop.address, stop.rider_names, stop.rider_count, stop.drive_time],
            1,
        ):
            cell = ws.cell(row=row, column=ci, value=val)
...
def generate_routes(
    ...
    if progress_cb:
        ...
        progress_cb(
            f"Loaded {len(students)} students across {unique} addresses, "
    ...
    wb = Workbook()
    build_dashboard(
        wb, vehicles, camp_address=camp_address, trip_direction=trip_direction
    )
    for veh in vehicles:
        build_vehicle_sheet(
            wb, veh, camp_address=camp_address, trip_direction=trip_direction
        )
